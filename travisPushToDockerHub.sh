...
./citools/retagApplications.sh "$INTEGRATION_APPS" $integrationtag ${toEnvironment}_latest push

echo "Tag and push all application images for deployment timestamp ${toEnvironment}__${DATETIME}"
./citools/retagApplications.sh "$INTEGRATION_APPS" $integrationtag ${toEnvironment}_${DATETIME} push
