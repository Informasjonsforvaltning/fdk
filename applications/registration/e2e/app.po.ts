...
export class RegistreringGuiPage {
  ...
  async createDataset(name) {
    ...
    await browser.wait(this.EC.presenceOf(newDatasetButton), 10000, "Coud not find button for creating a new dataset");
    await newDatasetButton.click();

    let section = element(by.css("label[for='title-and-description-toggle']"));
    await browser.wait(this.EC.presenceOf(section), 10000, "Coud not find section for title-and-description");
