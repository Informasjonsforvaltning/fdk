...
const CompareTermModalContent = (props) => {
  ...
  const title = items => items.map( (item, index) => (
    <div className={cols} key={`title-${  index  }${item.uri}`}>
      <h3>{
        (item.prefLabel && getTranslateText(item.prefLabel, selectedLanguageCode)) ?
          getTranslateText(item.prefLabel, selectedLanguageCode).charAt(0).toUpperCase() + getTranslateText(item.prefLabel, selectedLanguageCode).substring(1).toLowerCase() : ''
      }</h3>
      <h5>{
        (item.creator && item.creator.name) ? 
          item.creator.name.charAt(0).toUpperCase() + item.creator.name.substring(1) : ''
      }</h5>
    </div>
  ));

  const definition = items => items.map( (item, index) => (    
    <div className={cols} key={`definition-${  index  }${item.uri}`}>
      <h5>{localization.compare.definition}</h5>
      <p>{
        (item.definition && getTranslateText(item.definition, selectedLanguageCode)) ? 
          getTranslateText(item.definition, selectedLanguageCode).charAt(0).toUpperCase() + getTranslateText(item.definition, selectedLanguageCode).substring(1) : ''
      }</p>
    </div>
  ));
  ...
  for (let i=0; i<terms.length; i+=1) {      
    ...
    if (terms[i].inScheme) {        
      for (let j=0; j<terms[i].inScheme.length; j+=1) {
        inScheme += terms[i].inScheme[j].substring(terms[i].inScheme[j].lastIndexOf('/') + 1);
  ...
  const inScheme = items => items.map( (item, index) => (      
  ...
  const source = items => items.map( (item, index) => (      
  ...
  const note = items => items.map( (item, index) => (     
    ...
    if (terms[i].altLabel) {
      for (let j=0; j<terms[i].altLabel.length; j+=1) {          
        if (getTranslateText(terms[i].altLabel[j], selectedLanguageCode)) {
          altLabel += getTranslateText(terms[i].altLabel[j], selectedLanguageCode);
  ...
  const altLabel = items => items.map( (item, index) => (        
