export default [
  {
    key: '/STAT',
    doc_count: 91,
    children: [
      {
        key: '/STAT/972417807',
        doc_count: 37,
        hasParent: true,
        children: [
          {
            key: '/STAT/972417807/974761076',
            doc_count: 36,
            hasParent: true,
            children: []
          },
          {
            key: '/STAT/972417807/971526920',
            doc_count: 1,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/912660680',
        doc_count: 28,
        hasParent: true,
        children: [
          {
            key: '/STAT/912660680/974760673',
            doc_count: 27,
            hasParent: true,
            children: []
          },
          {
            key: '/STAT/912660680/971526157',
            doc_count: 1,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/872417842',
        doc_count: 9,
        hasParent: true,
        children: [
          {
            key: '/STAT/872417842/960885406',
            doc_count: 8,
            hasParent: true,
            children: []
          },
          {
            key: '/STAT/872417842/970018131',
            doc_count: 1,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/983887406',
        doc_count: 9,
        hasParent: true,
        children: [
          {
            key: '/STAT/983887406/915933149',
            doc_count: 8,
            hasParent: true,
            children: []
          },
          {
            key: '/STAT/983887406/983544622',
            doc_count: 1,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/983887457',
        doc_count: 4,
        hasParent: true,
        children: [
          {
            key: '/STAT/983887457/889640782',
            doc_count: 4,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/972417874',
        doc_count: 2,
        hasParent: true,
        children: [
          {
            key: '/STAT/972417874/985399077',
            doc_count: 2,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/972417858',
        doc_count: 1,
        hasParent: true,
        children: [
          {
            key: '/STAT/972417858/991825827',
            doc_count: 1,
            hasParent: true,
            children: []
          }
        ]
      },
      {
        key: '/STAT/972417866',
        doc_count: 1,
        hasParent: true,
        children: [
          {
            key: '/STAT/972417866/961181399',
            doc_count: 1,
            hasParent: true,
            children: []
          }
        ]
      }
    ]
  },
  {
    key: '/KOMMUNE',
    doc_count: 37,
    children: [
      {
        key: '/KOMMUNE/958935420',
        doc_count: 37,
        hasParent: true,
        children: [
          {
            key: '/KOMMUNE/958935420/974770482',
            doc_count: 22,
            hasParent: true,
            children: [
              {
                key: '/KOMMUNE/958935420/974770482/971040823',
                doc_count: 22,
                hasParent: true,
                children: []
              }
            ]
          },
          {
            key: '/KOMMUNE/958935420/974770474',
            doc_count: 10,
            hasParent: true,
            children: [
              {
                key: '/KOMMUNE/958935420/974770474/996922766',
                doc_count: 8,
                hasParent: true,
                children: []
              },
              {
                key: '/KOMMUNE/958935420/974770474/976820088',
                doc_count: 2,
                hasParent: true,
                children: []
              }
            ]
          }
        ]
      }
    ]
  },
  {
    key: '/PRIVAT',
    doc_count: 2,
    children: [
      { key: '/PRIVAT/985821585', doc_count: 1, hasParent: true, children: [] },
      { key: '/PRIVAT/991609407', doc_count: 1, hasParent: true, children: [] }
    ]
  },
  {
    key: '/ANNET',
    doc_count: 1,
    children: [
      { key: '/ANNET/910244132', doc_count: 1, hasParent: true, children: [] }
    ]
  }
];
